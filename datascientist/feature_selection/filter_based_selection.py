...
class PearsonCorrelation:
    ...
    def top_corr_featurenames(self, feat_num=1, ascending=True):
        ...
        cor_list = [0 if np.isnan(i) else i for i in cor_list]
        cor_list = np.around(cor_list, decimals=6)
        cor_feature = self.X.iloc[:,
                                  np.argsort(np.abs(cor_list))[-(feat_num):]]
        cor_feature = cor_feature.columns.tolist()
...
class ChiSquare:
    ...
    def chi2_score(self, sort=False, reset_index=False):
        ...
        Returns
        -------
        chi2_score: DataFrame with one column as the features name and the
        other as Chi-square statistics.
        '''
